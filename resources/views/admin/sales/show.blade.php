        ...
        <div class="bg-white rounded-xl shadow-card overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('admin.sales.receipt', $sale) }}" target="_blank" class="px-3 py-1.5 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Print Receipt</a>
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                @if($sale->location)
                <div><p class="text-sm text-gray-500">Location</p><p class="font-medium text-gray-900">{{ $sale->location->name }} <span class="text-xs text-gray-500">({{ ucfirst($sale->location->pos_type) }})</span></p></div>
                @endif
                ...
                <div><p class="text-sm text-gray-500">Source</p><p class="font-medium text-gray-900">
                    @if($sale->source === 'customer_app')
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800"><i class="fas fa-mobile-alt mr-1"></i> Customer App - Wholesale</span>
                    @else
            ...
            <div class="p-6 overflow-x-auto">
                <table class="w-full"><thead><tr class="border-b border-gray-200"><th class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Product</th><th class="text-right text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Qty</th><th class="text-right text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Price</th><th class="text-right text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Disc</th><th class="text-right text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Tax</th><th class="text-right text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-2">Total</th></tr></thead>
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr><td class="py-2 px-2 font-medium">{{ $item->product->name ?? 'N/A' }}{{ $item->variant ? ' (' . $item->variant->label . ')' : '' }}</td><td class="py-2 px-2 text-right">{{ number_format($item->quantity, 2) }}</td><td class="py-2 px-2 text-right">Rs. {{ number_format($item->unit_price, 2) }}</td><td class="py-2 px-2 text-right">Rs. {{ number_format($item->discount, 2) }}</td><td class="py-2 px-2 text-right">Rs. {{ number_format($item->tax, 2) }}</td><td class="py-2 px-2 text-right font-medium">Rs. {{ number_format($item->total_price, 2) }}</td></tr>
                        @endforeach
                    </tbody>
